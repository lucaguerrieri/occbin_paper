function [R kap_difference a_difference c] = rfuncirr(init,shock,state,irfshock,theta,P,modnam,modnamstar,constraint,constraint_relax,oobase_,Mbase_,oostar_,Mstar_)
...
kap_difference = zdataconcatenated(1,kpos);
kap_prime=exp(zdataconcatenated(1,kpos)+log(kss));
...
for nextstate = 1:nstates
    ...
    kap_2prime=exp(zdataconcatenated(1,kpos)+log(kss));
