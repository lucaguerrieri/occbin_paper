...
function Rvec = rfuncirr2(avec,nodes,theta,P,kstart,kend)
%
...
%nnegs = length(find(theta<0)); % number of states in which tech is below SS
nnegs = nstates;

order = length(avec)/(nstates+nnegs)-1;
...
Rvec = zeros(npoints,1);
a1 = reshape(avec(1:((order+1)*nstates)),order+1,nstates);
a2 = reshape(avec(((order+1)*nstates+1):end),order+1,nnegs);
